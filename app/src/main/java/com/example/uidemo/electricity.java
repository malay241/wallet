...
public class electricity extends AppCompatActivity {
    ...
    private boolean validateAmount(String amount) {
        if(amount.trim().equals("")){
            ...
            amount_Layout.setError("Amount can not be greater than 10000");
            return false;
        }
        else {
    ...
    private boolean validatePhone(String phone) {
        ...
        if(phone.trim().equals("")){
            phone_Layout.setError("Account number can't be left empty!");
            return false;
        }
        else if (phone.trim().length()==10 && android.util.Patterns.PHONE.matcher(phone).matches()) {
            phone_Layout.setError(null);
            return true;
        }
        else{
            phone_Layout.setError("Enter valid account number!");
            return false;
        }
    }
    ...
    public void openbillreview(View view) {
        if(flagele && flagamount) {
            Intent intent = new Intent(getApplicationContext(), ElectricityBillReview.class);
            startActivity(intent);
        }
    }
}